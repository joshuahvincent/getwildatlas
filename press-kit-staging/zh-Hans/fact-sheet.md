# Wild Atlas — 信息速览

**标语：** 培养探险家，而非低头族。

---

## 基本信息

| | |
|---|---|
| **应用名称** | Wild Atlas |
| **平台** | iOS — iPhone 和 iPad |
| **下载渠道** | App Store — 2026 年 6 月 10 日上线 |
| **适用年龄** | 3–9 岁 |
| **开发者** | Joshuah Vincent，美国华盛顿州西雅图 |
| **App Store 链接** | https://apps.apple.com/app/id6761081031 |
| **媒体联系** | press@wildatlasapp.com |
| **新闻资料包** | wildatlasapp.com/press |

---

## 定价

| | 价格 |
|---|---|
| 免费动物包（3 个，永久免费） | 免费 |
| 单个付费动物包 | ¥28 |
| Explorer Pass（全部 10 个付费包） | 见 App Store |

- 一次性购买，需家长确认
- 无订阅
- 无广告
- 不采集数据
- 符合 COPPA 规定
- 独立开发并自筹资金——无外部投资人

---

## 内容

- **236 种动物**，分布于 **13 个动物包**，涵盖各大洲和海洋
- **3 个永久免费动物包：** Happy Hounds、Cool Cats、Cozy Critters
- **1 个免费礼赠包：** 新用户在首次设置时可自选一个付费包免费解锁
- **10 个付费动物包**（单独购买或通过 Explorer Pass 打包）
- **隐藏彩蛋包：** Mythical Menagerie（18 种世界民间传说生物——在应用多次使用中发现隐藏的流星即可解锁）

### 功能特点

- 全程语音讲述——无需识字，3 岁即可独立使用
- 双模式问答：图片模式（适合年幼儿童）和阅读模式（适合年长儿童），多种难度级别
- 互动世界地图——每种动物在其真实栖息地标注
- 体型与体重对比
- Wild Watch 幻灯片：Tiny Tails、Sized Up、Weigh to Go、Wild & Free
- Games Den：Memory Match、Puppy Puzzle、Who's Who、Habitat Hop（4 款小游戏）
- 完全支持离线使用

### 支持语言

六种语言同步上线：英语（美国）、英语（英国）、德语、西班牙语、法语、简体中文——每种语言均完整配音。

---

## 关于开发者

Joshuah Vincent 是一位居住在西雅图的产品开发者，两个孩子的父亲。他在 Meta 担任首席产品经理六年，主导推出了 Ray-Ban Meta 和 Orion 等 AR 产品，此前在 Zillow 和 Xero 担任产品负责人。Wild Atlas 独立开发并自筹资金，无外部投资人。

---

## 家长们怎么说

> ★★★★★ **Educational excellence**
>
> "This app has such a wide variety of features that are both engaging and fun for children! Love the dinosaur pack"
>
> — kula131, Apple App Store

> ★★★★★ **Wonderful app**
>
> "In a world where apps are meant for mindless scrolling, Wild Atlas stands apart. It is fun, educational, and beautiful. It's not really a game, it's more of an animal encyclopedia for kids. It's the only app I let my 3 year old play."
>
> — chu.c.me, Apple App Store

> ★★★★★ **Not brain rot!**
>
> "This is actually a good game that teaches kids facts about animals without a bunch of addictive tricks many games use to compensate for being otherwise boring."
>
> — Happy Campering, Apple App Store

*五星评价原文摘自 Apple App Store。*
